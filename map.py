...
class Map:
    def __init__(self):
        self.surface = pg.Surface((config.UNSCALED_SIZE[0] + 2 * config.TILE_SIZE,
                                   config.UNSCALED_SIZE[1] + 2 * config.TILE_SIZE))
    ...
    def generate_background(self):
        ...
        addon = 0
        for x in range(0, 3 + config.UNSCALED_SIZE[0]//config.TILE_SIZE):
            if addon == 0:
                addon = 1
            else:
                addon = 0
            for y in range(0, 3 + config.UNSCALED_SIZE[1] // config.TILE_SIZE, 2):
